...
privileged aspect UsuarioController_Roo_Controller_Json {
    ...
    @RequestMapping(params = "find=ByCodigoPinAndTelefone", headers = "Accept=application/json")
    @ResponseBody
    public ResponseEntity<String> UsuarioController.jsonFindUsuariosByCodigoPinAndTelefone(@RequestParam("codigoPin") CodigoPIN codigoPin, @RequestParam("telefone") Telefone telefone) {
        HttpHeaders headers = new HttpHeaders();
        try {
            headers.add("Content-Type", "application/json; charset=utf-8");
            return new ResponseEntity<String>(Usuario.toJsonArray(Usuario.findUsuariosByCodigoPinAndTelefone(codigoPin, telefone).getResultList()), headers, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<String>("{\"ERROR\":"+e.getMessage()+"\"}", headers, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
