...
public class AgenciaServiceImpl implements AgenciaService {
	...
	public void cadastrar(Agencia agencia) throws CashPackException {
		...
		if (usuarioComMesmoTelefone != null) {
			if(usuarioComMesmoTelefone instanceof Agencia){
				...
				throw new AgenciaException("O telefone informado está vinculado a um usuário já cadastrado!");
			}
		}

		if (agenciaPesquisadaPorTelefone != null && agenciaPesquisadaPorTelefone.getId() != null) {
		...
		verificarAgenciaComPinEmitidoRecentemente(agenciaPesquisadaPorTelefone);
	...
	private void verificarAgenciaComPinEmitidoRecentemente(Agencia agencia) throws CodigoPinEmitidoRecentementeException {
	...
	private void verificarTempoDeEnvioDoUltimoPin(Agencia agencia)
		...
		Date dataQuePinFoiEnviado = agencia.getCodigoPin().getDataQueFoiGerado();
		...
		int diferencaEntreAno = calendarDoMomentoQueFoiEnviado.get(Calendar.YEAR) - calendarAtual.get(Calendar.YEAR);
		int diferencaEntreMes = calendarDoMomentoQueFoiEnviado.get(Calendar.MONTH) - calendarAtual.get(Calendar.MONTH);
		int diferencaEntreDias = calendarDoMomentoQueFoiEnviado.get(Calendar.DAY_OF_MONTH) - calendarAtual.get(Calendar.DAY_OF_MONTH);

		if (diferencaEntreAno == 0 && diferencaEntreMes == 0 && diferencaEntreDias == 0) {
			throw new CodigoPinEmitidoRecentementeException("Tempo mínimo para emissão de um novo PIN ainda não foi atingido! Tempo mínimo: 1 dia");
		}
	}

	private Agencia findAgenciaByNumeroDoDocumento(String numeroDoDocumento) {
		return Agencia.findAgenciaByNumeroDocumento(numeroDoDocumento);
	}

	private Usuario findUsuarioByTelefone(String codPais, String codArea,
			String numero) {

		return Usuario.findUsuarioByCodPaisAndCodAreaAndNumero(codPais, codArea, numero);
	...
	private void validate(Agencia agencia) throws CashPackException {
		...
		if (agencia.getTipoDeDocumentoAgenciaEnum().equals(TipoDeDocumentoDaAgenciaEnum.CPF) && agencia.getNumeroDocumento().length() != 11) {
		...
		if (agencia.getRamoDeAtividade() == null || agencia.getRamoDeAtividade().getId() == null) {
			...
			RamoDeAtividade ramo = RamoDeAtividade.findRamoDeAtividade(agencia.getRamoDeAtividade().getId());
			
			if (ramo == null) {
				throw new AgenciaException("Ramo de Atividade não está cadastrado no sistema!");
			} else {
	...
	public void confirmarPinAgencia(Agencia agencia) throws CashPackException {
		...
		try {
			Telefone telefone = agencia.getTelefone();
			usuario = this.findUsuarioByTelefone(telefone.getCodPais(), telefone.getCodArea(), telefone.getNumero());
			...
			throw new AgenciaException("Não existe agência cadastrada com o telefone informado!");
		}

		if (!agenciaPesquisada.getStatusAgencia().equals(StatusAgencia.DESATIVADO)) {
			throw new CodigoPINJaAtivadoException("Agência já teve o PIN ativado!");
		}

		if (!agenciaPesquisada.getCodigoPin().getCodigo().toUpperCase().equals(agencia.getCodigoPin().getCodigo().toUpperCase())) {
